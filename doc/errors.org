...
Emacs Lisp's [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Handling-Errors.html][error handling mechanism]] uses [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Nonlocal-Exits.html][non-local exits]]. Rust uses
=Result= enum. =rem= converts between the 2 at the Rust-Lisp
boundaries (more precisely, Rust-C).

The chosen error type is the =Box<dyn Error>= object:
...
Note the use of =unsafe= to extract the error symbol as a =Value=. The
reason is that, =ErrorKind::Signal= is marked =Send+Sync=, for
compatibility with =std::error::Error=, while =Value= is
lifetime-bound by =env=. The =unsafe= contract here requires the error
being handled (and its =TempValue=) to come from this =env=, not from
another thread, or from a global/thread-local storage.

*** Catching Values Thrown by Lisp

This is similar to handling Lisp errors. The only difference is
=ErrorKind::Throw= being used instead of =ErrorKind::Signal=.

** Signaling Lisp Errors from Rust

The function =env.signal= allows signaling a Lisp error from Rust
code. The error symbol must have been defined, e.g. by the macro
=define_errors!=:
...
In addition to [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Standard-Errors.html][standard errors]], Rust module functions can signal
Rust-specific errors, which can also be handled by =condition-case=:
...
by using =catch_unwind= at the Rust-to-C boundary to convert a panic
into a Lisp's signal/throw of the appropriate type:

- Normally the panic is converted into a Lisp's error signal of the
  type =rust-panic=. Note that it is **not a sub-type** of
  =rust-error=.
- If the panic value is an =ErrorKind=, it is converted to the
  corresponding signal/throw, as if a =Result= was returned. This
  allows propagating Lisp's non-local exits through contexts where
  =Result= is not appropriate, e.g. callbacks whose types are dictated
  by 3rd-party libraries, such as =tree-sitter=.
